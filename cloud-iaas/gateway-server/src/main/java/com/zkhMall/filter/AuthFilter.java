package com.zkhMall.filter;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.zkhMall.configuration.WhiteAllowUrl;
import com.zkhpower.constant.AuthConstants;
import com.zkhpower.constant.BusinessEnum;
import com.zkhpower.constant.HttpConstants;
import com.zkhpower.model.Result;
...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (whiteAllowUrl.getAllowUrl().contains(path)) {
            //请求path，包含在白名单中，放行
        ...
        //查看是否有值、redis中是否有值
        if (StringUtils.hasText(token) && redisTemplate.hasKey(AuthConstants.REDIS_LOGIN_PREFIX+token)) {
            //有值，且token存在于redis中
            return chain.filter(exchange);
        }
